...
def clear_disaster_area(sim):
    ...
    try:
        ...
        if disaster_group_handle != -1:
            ...
            # Very important: remove the dummy handle from child_handles if it is included
            child_handles = [h for h in child_handles if h != disaster_group_handle]

            # Remove all children first
            sim.removeObjects(child_handles)

            # Then remove the DisasterGroup dummy itself
            try:
...
def restart_disaster_area(sim, config):
    """
    Clears the DisasterGroup, then asks user whether to create a new disaster.
    """
